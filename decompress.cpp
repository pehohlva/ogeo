...
#define ZIP_VERSION 20
    ...
    QByteArray Stream::fileByte(const QString &fileName) {
        ...
        qWarning() << "Zip: Unknown compression method";
        return QByteArray();
    }
    ...
    /// modus 1 = openoffice file only write on disk valid XML
    /// modus 9 = geoip csv file all on dir path
    /// modus 0 = default dep down dir zip struct..
    void Stream::explode_todir(const QString path, int modus) {
        ...
        while (i.hasNext()) {
            ...
            if (pfile.absolutePath() != pfile.absoluteFilePath()) {
                filecount++;
                QString wfile = path + pfile.fileName(); 
                if ( modus == 0 ) {
                    wfile = pfile.absoluteFilePath();
                }
                QByteArray data = i.value();
                if ( modus == 9 || modus == 0 ) {
                    /// all file on path given no subdir 
                            RamBuffer *buffercsv = new RamBuffer("csv");
                            buffercsv->device()->write(data);
                            buffercsv->flush_onfile(wfile);
                            buffercsv->clear();
                            
                } else {
                    // _write_file xml file is xml 
        ...
        qDebug() << "Zip Writtel total " << filecount << " file on path:" << path;
    }
...
#define CHUNK 16384
    ...
    //// doc http://qt4-x11.sourcearchive.com/documentation/4.5.3really4.5.2/qhttpnetworkreply_8cpp_source.html

    qint64 inflate_dual(QByteArray &compressed, QByteArray &inflated) {
        qint64 ret = Z_DATA_ERROR;
        const int gzcomp_size = compressed.size();
        unsigned have;
        ...
        bool streamEnd = (ret == Z_STREAM_END);
        if (!streamEnd) {
            qDebug() << "###  inflateStrm error .... \n";
        }
    ...
    QByteArray deflate_gz(const QByteArray chunk) {
        QTemporaryFile file;
         ...
         /// QString QDir::tempPath() //// .toStdString();
        const char *tmpgzfile = qPrintable(file.fileName()); 
        QByteArray input;
        if (file.open()) {
            file.write(chunk);
            file.close();
            ...
            if (!filegunzip) {
                qDebug() << "### Unable to work on tmp file ... " << QDir::tempPath();
                return QByteArray();
            }
            char buffer[1024];
...
namespace Zip {
    ...
    Zip::FileInfo::FileInfo(const FileInfo &other) {
        operator=(other);
    }

    Zip::FileInfo& Zip::FileInfo::operator=(const FileInfo &other) {
